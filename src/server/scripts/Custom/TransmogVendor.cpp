...
public:
    ...
    void OnSave(Player* player) override
    {
        ...
        trans->PAppend("DELETE FROM `custom_transmogrification_vendor` WHERE `Owner` = %u", lowguid);
        ...
        if (!player->transmogMap.empty())
        {
            ...
            for (std::vector<ObjectGuid>::const_iterator it = items.begin(); it != items.end(); ++it)
            {
                ...
                trans->PAppend("REPLACE INTO custom_transmogrification_vendor (GUID, FakeEntry, Owner) VALUES (%u, %u, %u)", it2->first.GetCounter(), it2->second, lowguid);
            }
        }
    ...
    {
        QueryResult result = CharacterDatabase.PQuery("SELECT GUID, FakeEntry FROM custom_transmogrification_vendor WHERE Owner = %u", player->GetGUID().GetCounter());
...
public:
    ...
    void OnStartup() override
    {
        ...
        CharacterDatabase.DirectExecute("DELETE FROM custom_transmogrification_vendor WHERE NOT EXISTS (SELECT 1 FROM item_instance WHERE item_instance.guid = custom_transmogrification_vendor.GUID)");
#endif
    }
